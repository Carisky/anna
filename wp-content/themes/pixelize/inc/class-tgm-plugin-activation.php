...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function init() {
			...
			// Load class strings.
			$this->strings = array(
				'page_title'                      => __( 'Install Required Plugins', 'pixelize' ),
				'menu_title'                      => __( 'Install Plugins', 'pixelize' ),
				/* translators: %s: plugin name. */
				'installing'                      => __( 'Installing Plugin: %s', 'pixelize' ),
				/* translators: %s: plugin name. */
				'updating'                        => __( 'Updating Plugin: %s', 'pixelize' ),
				'oops'                            => __( 'Something went wrong with the plugin API.', 'pixelize' ),
				...
				'return'                          => __( 'Return to Required Plugins Installer', 'pixelize' ),
				'dashboard'                       => __( 'Return to the Dashboard', 'pixelize' ),
				'plugin_activated'                => __( 'Plugin activated successfully.', 'pixelize' ),
				...
				'dismiss'                         => __( 'Dismiss this notice', 'pixelize' ),
		...
		public function filter_plugin_action_links_update( $actions ) {
			$actions['update'] = sprintf(
				...
				esc_attr__( 'This plugin needs to be updated to be compatible with your theme.', 'pixelize' ),
				esc_html__( 'Update Required', 'pixelize' )
			);
				...
				if ( ! current_user_can( 'activate_plugins' ) && ! current_user_can( 'install_plugins' ) && ! current_user_can( 'update_plugins' ) ) {
					...
					foreach ( $message as $type => $plugin_group ) {
						...
						$rendered .= sprintf(
							$line_template,
							sprintf(
								translate_nooped_plural( $this->strings[ $type ], $count, 'pixelize' ),
								$imploded,
								$count
							)
						);
		...
		protected function create_user_action_links_for_notice(
			...
			if ( current_user_can( 'install_plugins' ) ) {
				if ( $install_count > 0 ) {
					$action_links['install'] = sprintf(
						$link_template,
						translate_nooped_plural( $this->strings['install_link'], $install_count, 'pixelize' ),
				...
				if ( $update_count > 0 ) {
					$action_links['update'] = sprintf(
						$link_template,
						translate_nooped_plural( $this->strings['update_link'], $update_count, 'pixelize' ),
						esc_url( $this->get_tgmpa_status_url( 'update' ) )
					);
				}
			}
			...
			if ( current_user_can( 'activate_plugins' ) && $activate_count > 0 ) {
				$action_links['activate'] = sprintf(
					$link_template,
					translate_nooped_plural( $this->strings['activate_link'], $activate_count, 'pixelize' ),
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function get_plugin_advise_type_text( $required ) {
			if ( true === $required ) {
				return __( 'Required', 'pixelize' );
			}

			return __( 'Recommended', 'pixelize' );
		}
		...
		protected function get_plugin_source_type_text( $type ) {
			...
			switch ( $type ) {
				case 'repo':
					$string = __( 'WordPress Repository', 'pixelize' );
					break;
				case 'external':
					$string = __( 'External Source', 'pixelize' );
					break;
				case 'bundled':
					$string = __( 'Pre-Packaged', 'pixelize' );
					break;
			}
		...
		protected function get_plugin_status_text( $slug ) {
			if ( ! $this->tgmpa->is_plugin_installed( $slug ) ) {
				return __( 'Not Installed', 'pixelize' );
			}

			if ( ! $this->tgmpa->is_plugin_active( $slug ) ) {
				$install_status = __( 'Installed But Not Activated', 'pixelize' );
			} else {
				$install_status = __( 'Active', 'pixelize' );
			}

			$update_status = '';

			if ( $this->tgmpa->does_plugin_require_update( $slug ) && false === $this->tgmpa->does_plugin_have_update( $slug ) ) {
				$update_status = __( 'Required Update not Available', 'pixelize' );

			} elseif ( $this->tgmpa->does_plugin_require_update( $slug ) ) {
				$update_status = __( 'Requires Update', 'pixelize' );

			} elseif ( false !== $this->tgmpa->does_plugin_have_update( $slug ) ) {
				$update_status = __( 'Update recommended', 'pixelize' );
			}
			...
			/* translators: 1: install status, 2: update status */
				_x( '%1$s, %2$s', 'Install/Update Status', 'pixelize' ),
				$install_status,
				$update_status
			);
		}
		...
		public function get_columns() {
			$columns = array(
				...
				'plugin' => __( 'Plugin', 'pixelize' ),
				'source' => __( 'Source', 'pixelize' ),
				'type'   => __( 'Type', 'pixelize' ),
			);

			if ( 'all' === $this->view_context || 'update' === $this->view_context ) {
				$columns['version'] = __( 'Version', 'pixelize' );
				$columns['status']  = __( 'Status', 'pixelize' );
			}
		...
		protected function get_row_actions( $item ) {
			...
			if ( ! $this->tgmpa->is_plugin_installed( $item['slug'] ) ) {
				...
				$actions['install'] = __( 'Install %2$s', 'pixelize' );
			} else {
				// Display the 'Update' action link if an update is available and WP complies with plugin minimum.
				if ( false !== $this->tgmpa->does_plugin_have_update( $item['slug'] ) && $this->tgmpa->can_plugin_update( $item['slug'] ) ) {
					/* translators: %2$s: plugin name in screen reader markup */
					$actions['update'] = __( 'Update %2$s', 'pixelize' );
				}

				// Display the 'Activate' action link, but only if the plugin meets the minimum version.
				if ( $this->tgmpa->can_plugin_activate( $item['slug'] ) ) {
					/* translators: %2$s: plugin name in screen reader markup */
					$actions['activate'] = __( 'Activate %2$s', 'pixelize' );
				}
			}
		...
		public function wp_plugin_update_row( $slug, $item ) {
			...
			esc_html__( 'Upgrade message from the plugin author:', 'pixelize' ),
		...
		public function get_bulk_actions() {
			...
			if ( 'update' !== $this->view_context && 'activate' !== $this->view_context ) {
				if ( current_user_can( 'install_plugins' ) ) {
					$actions['tgmpa-bulk-install'] = __( 'Install', 'pixelize' );
				}
			}

			if ( 'install' !== $this->view_context ) {
				if ( current_user_can( 'update_plugins' ) ) {
					$actions['tgmpa-bulk-update'] = __( 'Update', 'pixelize' );
				}
				if ( current_user_can( 'activate_plugins' ) ) {
					$actions['tgmpa-bulk-activate'] = __( 'Activate', 'pixelize' );
				}
			}
			...
			if ( 'tgmpa-bulk-install' === $this->current_action() || 'tgmpa-bulk-update' === $this->current_action() ) {
				...
				if ( empty( $_POST['plugin'] ) ) {
					if ( 'install' === $install_type ) {
						$message = __( 'No plugins were selected to be installed. No action taken.', 'pixelize' );
					} else {
						$message = __( 'No plugins were selected to be updated. No action taken.', 'pixelize' );
					}
				...
				if ( empty( $plugins_to_install ) ) {
					if ( 'install' === $install_type ) {
						$message = __( 'No plugins are available to be installed at this time.', 'pixelize' );
					} else {
						$message = __( 'No plugins are available to be updated at this time.', 'pixelize' );
					}
			...
			if ( ! class_exists( 'TGMPA_Bulk_Installer' ) ) {
				...
				class TGMPA_Bulk_Installer extends Plugin_Upgrader {
					...
					public function activate_strings() {
						$this->strings['activation_failed']  = __( 'Plugin activation failed.', 'pixelize' );
						$this->strings['activation_success'] = __( 'Plugin activated successfully.', 'pixelize' );
					}
