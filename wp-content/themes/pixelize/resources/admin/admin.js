...
/*!**************************************!*\
  ...
  \**************************************/
(function ($) {
  var $notice_container = $(".pixelize-admin-big-notice--container");
  var selectedFrontPage = 0;

  if (!window.pixelize_admin) {
    return;
  }

  var _window$pixelize_admi = window.pixelize_admin,
      builderStatusData = _window$pixelize_admi.builderStatusData,
      getStartedData = _window$pixelize_admi.getStartedData;
  ...
  function toggleProcessing(value) {
    $(window).on("beforeunload.pixelize-admin-big-notice", function () {
      return true;
    });

    if (value) {
      $(".pixelize-admin-big-notice").addClass("processing");
      $(".pixelize-admin-big-notice .action-buttons").fadeOut();
    } else {
      $(".pixelize-admin-big-notice").removeClass("processing");
    }
  }
  ...
  function installBuilder(callback) {
    ...
    $.get(builderStatusData.install_url).done(function () {
      ...
      $(window).off("beforeunload.pixelize-admin-big-notice");
    });
  }
  ...
  $notice_root = $notice_container.closest(".pixelize-admin-big-notice");
  $custom_close_button = $notice_root.find(".pixelize-notice-dont-show-container");

  if ($custom_close_button.length) {
    $custom_close_button.on("click", function () {
      disableNotice();
      $notice_container.closest(".pixelize-admin-big-notice").find("button.notice-dismiss").click();
    });
  } else {
  ...
  window.pixelize_admin.showOverlay = showOverlay;
})(jQuery);
/******/ })()
;
