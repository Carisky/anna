...
wp_localize_script(
	...
	array(
		...
		'builderStatusData' => array(
			'status'                          => pixelize_theme()->getPluginsManager()->getPluginState( pixelize_get_builder_plugin_slug() ),
			'install_url'                     => pixelize_theme()->getPluginsManager()->getInstallLink( pixelize_get_builder_plugin_slug() ),
			'activate_url'                    => pixelize_theme()->getPluginsManager()->getActivationLink( pixelize_get_builder_plugin_slug() ),
			'slug'                            => pixelize_get_builder_plugin_slug(),
...
<div class="pixelize-admin-big-notice--container">
	<div class="logo-holder">
        <h2><?php echo sprintf(__("Choose one of the %s predefined designs to start with:", 'pixelize'), ucfirst(get_stylesheet())); ?></h2>
	</div>
	<div class="content-holder">
		<ul class="predefined-front-pages">
            ...
            $front_page_designs =   array(
                array(
                    'name'    => __( 'Modern', 'pixelize' ),
                    'index'   => 1,
                    'preview' => 'front-page-1.jpg',
                ),

                array(
                    'name'    => __( 'Fullscreen', 'pixelize' ),
                    'index'   => 2,
                    'preview' => 'front-page-2.jpg',
                ),

                array(
                    'name'    => __( 'Classic', 'pixelize' ),
                    'index'   => 3,
            ...
            foreach ( $front_page_designs as $pixelize_design_index => $design ) : ?>
				<?php
					$pixelize_design_selected = $pixelize_design_index === 0 ? 'selected' : '';
				...
				<li data-index="<?php echo esc_attr( $design['index'] ); ?>"
					class="<?php echo $pixelize_design_selected; ?>">
                    ...
                    <div class="predefined-front-page-card">
						...
						<div class="front-page-design-wrapper">
							<div class="design-preview-image"
								 style="background-image: url(<?php echo esc_attr( pixelize_theme()->getAssetsManager()->getBaseURL() . "/images/{$preview_image_name}" ); ?>)"
							></div>
						</div>
