...
function pixelize_register_components( $components ) {
...
Hooks::prefixed_add_filter( 'components', 'pixelize_register_components', 20 );
Theme::load(
	array(
 ...
 * @return Theme
 */
function pixelize_theme() {
	return Theme::getInstance();
}

function pixelize_assets() {
	return pixelize_theme()->getAssetsManager();
}
...
add_action('after_setup_theme', function() {
    pixelize_theme()->register_menus(
        array(
            'header-menu' => esc_html__( 'Header Menu', 'pixelize' ),
            'footer-menu' => esc_html__( 'Footer Menu', 'pixelize' ),
        )
    );
}, 1);
...
if ( ! apply_filters( 'kubio_is_enabled', false ) ) {
	pixelize_assets()
		...
		->registerStylesheet( 'pixelize-theme', '/theme/theme.css' );
	pixelize_assets()->loadLocalGoogleFonts();
	//	foreach ( Defaults::get( 'fonts', array() ) as $family => $variants ) {
	//		pixelize_assets()->addGoogleFont( $family, $variants );
	//	}

	add_action( 'wp_enqueue_scripts', 'pixelize_print_color_scheme', 0 );
} else {
	pixelize_assets()
		->registerStyle( 'pixelize-theme', pixelize_assets()->getBaseURL() . '/theme/fse-base-style.css' );
	Hooks::prefixed_add_filter( 'skip_google_fonts', '__return_true' );
}

function pixelize_theme_add_woocommerce_support() {
	add_theme_support(
		'woocommerce',
		array(
			'product_grid' => array(
...
add_action( 'after_setup_theme', 'pixelize_theme_add_woocommerce_support' );

add_filter( 'kubio/activation/after_activation_redirect_url', 'pixelize_after_activation_redirect_url' );
function pixelize_after_activation_redirect_url( $url ) {
...
function pixelize_get_builder_plugin_slug() {
...
function pixelize_theme_plugins( $plugins ) {

	$plugin_slug = pixelize_get_builder_plugin_slug();

	$plugins[ $plugin_slug ] = array(
		'name'        => $plugin_slug === 'kubio-pro' ? __( 'Kubio PRO', 'pixelize' ) : __( 'Kubio', 'pixelize' ),
...
Hooks::prefixed_add_filter( 'theme_plugins', 'pixelize_theme_plugins' );
...
Hooks::prefixed_add_action(
	...
	function ( $slug ) {
		...
		if ( $slug === pixelize_get_builder_plugin_slug() ) {
                ...
                <div class="kubio-admin-row get-started-imported notice notice-success">
                    <div>
                        <p class="imported-title">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported!', 'pixelize' ),
									pixelize_theme()->getName()
								)
							);
							?>
                        </p>
                        <p class="imported-subtitle">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported! You can take a look at your new design or start editing it', 'pixelize' ),
									pixelize_theme()->getName()
								)
							);
							?>
                    ...
                    <div class="button imported-view-site-button">
                        <a href="<?php echo esc_url( site_url() ); ?>">
							<?php echo esc_html( __( 'View site', 'pixelize' ) ); ?>
                        </a>
...
function pixelize_try_online_url() {
	return 'https://kubiobuilder.com/go/try-theme/' . get_template();
}

add_filter( 'kubio/editor-try-online/url', 'pixelize_try_online_url', 10 );

function pixelize_render_header_style() {

	?>
    <base target="_top">
	<?php

	pixelize_theme()->get( 'css' )->render();
}
add_action( 'wp_head', 'pixelize_render_header_style', 100 );
